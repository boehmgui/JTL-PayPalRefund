...
//                IMAGE = 'cdrx/pyinstaller-linux:python3'
            ...
            steps {
                    // for Linux
                    //sh 'docker run --rm -e PYTHONDONTWRITEBYTECODE=1 -v "\$(pwd):/src" ${IMAGE} "python3 -m PyInstaller -F ./src/pp_refund.py"'

                    // for Windows
                    //sh 'docker run --rm -e PYTHONDONTWRITEBYTECODE=1 -v "\$(pwd):/src" ${IMAGE} "pyinstaller -F ./src/pp_refund.py"'
                    // should work for Windows _and_ Linux
                    sh 'docker run  --rm -e PYTHONDONTWRITEBYTECODE=1 -v "\$(pwd):/src" ${IMAGE} "pyinstaller --onefile ./src/pp_refund.py"'

                }
            post {
                success {
                    // for Linux
                    //archiveArtifacts "dist/pp_refund"
                    // for Windows
                    archiveArtifacts "dist/pp_refund.exe"
